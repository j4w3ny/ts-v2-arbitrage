...
import {Error} from "../V2LibError.sol";

import {
    TimeswapV2PoolMint,
    TimeswapV2PoolBurn,
    TimeswapV2PoolDeleverage,
    TimeswapV2PoolLeverage,
    TimeswapV2PoolRebalance,
    TransactionLibrary
} from "./enums/PoolTransaction.sol";
...
/// @param  saction The type of rebalance transaction, more information in Transaction module.
...
library ParamLibrary {
    ...
    function check(TimeswapV2PoolCollectProtocolFeesParam memory param) internal pure {
        if (param.long0To == address(0) || param.long1To == address(0) || param.shortTo == address(0)) {
            Error.zeroAddress();
        }
    ...
    function check(TimeswapV2PoolCollectTransactionFeesAndShortReturnedParam memory param) internal pure {
        if (
            param.long0FeesTo == address(0) || param.long1FeesTo == address(0) || param.shortFeesTo == address(0)
                || param.shortReturnedTo == address(0)
        ) Error.zeroAddress();
        if (param.maturity > type(uint96).max) Error.incorrectMaturity(param.maturity);
        if (
            (
                param.long0FeesRequested == 0 && param.long1FeesRequested == 0 && param.shortFeesRequested == 0
                    && param.shortReturnedRequested == 0
            ) || param.strike == 0
        ) Error.zeroInput();
    }
